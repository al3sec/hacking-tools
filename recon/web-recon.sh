...
echo "consulting crt.sh in order to fetch subdomains ..."
curl -s "https://crt.sh/?q=$1&output=json" | jq -r '.[].name_value' | sed 's/\*\.//g' | sort -u | tee "$domain-crtsh-output.txt"

cat "$domain-amass-output.txt" "$domain-subfinder-output.txt" "$domain-knockpy-output.txt"  "$domain-crtsh-output.txt" > "$domain-subdomains.txt"
...
cat "$domain-uniq-subdomains.txt" | puredns resolve --resolvers /opt/useful/resolvers.txt --write "$domain-subdomains-perm.txt" --rate-limit 500
...
echo "starting ffuf to bruteforce subdomains..."
...
echo "starting ffuf to bruteforce vhosts..."
ffuf -w /opt/useful/SecLists/Discovery/DNS/subdomains-top1million-110000.txt -u "https://$1" -H "Host: FUZZ.$1" -o "$domain-vhosts-json-ffuf-output.txt" -mc 200,204,301,302,307
cat "$domain-vhosts-json-ffuf-output.txt" | jq '.results[].url' > "$domain-ffuf-vhosts-output.txt"
sed -i -e 's/"//g' "$domain-ffuf-vhosts-output.txt"

# Adding ffuf output to online subdoms
cat "$domain-ffuf-output.txt" "$domain-ffuf-vhosts-output.txt" >> "$domain-online-subdomains.txt"
